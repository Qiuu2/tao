...
else
{
		...
		if($_POST['searchkey']!="")
		{
			if($_POST['searchkey']=="taskname")
			{
				if($_POST['searchsequence']!="")
				{
					...
					$sql="SELECT * FROM task WHERE tasktype IN (27)   and parentid='$get_task_id' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY '$_GET[searchsequence]' desc ";
					}
					else
					{
					$sql="SELECT * FROM task WHERE tasktype IN (27)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and parentid='$get_task_id' and task_user_id='$userid' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY '$_GET[searchsequence]' desc ";
					}
				}
				else
				{
					...
					if($_SESSION['username']=="admin")
					{
					$sql="SELECT * FROM task WHERE tasktype IN (27)   and  parentid='$get_task_id' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY playtime desc ";
					}
					else
					{
					$sql="SELECT * FROM task WHERE tasktype IN (27)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and  parentid='$get_task_id' and task_user_id='$userid' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY playtime desc ";
					}
				}
			}
			...
			if($_POST['searchkey']=="playtime")
			{
				if($_POST['searchsequence']!="")
				{
					...
					$sql="SELECT * FROM task WHERE tasktype IN (27) and parentid='$get_task_id' and  playtime >= '".trim($_POST['searchvalue'])."' ORDER BY playtime desc ";
					}
					else
					{
					$sql="SELECT * FROM task WHERE tasktype IN (27)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and parentid='$get_task_id' and task_user_id='$userid' and  playtime >= '".trim($_POST['searchvalue'])."' ORDER BY playtime desc ";
					}
				}
				else
				{
	...
	else
	{
		if($_SESSION['username']=="admin")
		{
		$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,username FROM task,book_admin WHERE task_user_id=book_admin.id AND tasktype IN (27)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') ORDER BY playtime desc";
		}
		else
		{
		$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,book_admin.username FROM task,book_admin WHERE task_user_id=book_admin.id  AND tasktype IN (27)  ORDER BY playtime desc";
		}
	}
...
/*
	...
	*/
	...
	$smarty->display("videotaskmanager/videoTaskManager.html");
}
?>
