...
else
{
	...
	if(trim($_GET['searchvalue'])!="")
	{
		if(trim($_GET['searchkey'])!="" && trim($_GET['searchsequence'])!="")
		{
			...
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity FROM terminal WHERE totalcapacity!=0 AND terminal.terminalname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ";
			}
			
			if(trim($_GET['searchkey'])=="ip")
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity FROM terminal WHERE totalcapacity!=0 AND terminal.ip LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ";
			}
		}
		else if(!empty($_GET['searchkey']) && empty($_GET['searchsequence']))
		{
			if(trim($_GET['searchkey'])=="terminalname")
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity FROM terminal WHERE totalcapacity!=0 AND terminal.terminalname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id DESC ";
			}
			
			if(trim($_GET['searchkey'])=="ip")
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity FROM terminal WHERE totalcapacity!=0 AND terminal.ip LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id DESC ";
			}
		}
		else
		{
	...
	if($user_name=="admin")
		$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity FROM terminal WHERE totalcapacity!=0 ORDER BY id ASC ";
	  else
	  	$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity  FROM terminal WHERE totalcapacity!=0 AND terminal.id IN(SELECT terminalid FROM userterminal WHERE userterminal.userid IN(SELECT id FROM book_admin WHERE book_admin.username = '$user_name')) ORDER BY IP ASC ";
	}
	...
	*/
	...
	$smarty->display("offlinetask/terminalmanager.html");
}
?>