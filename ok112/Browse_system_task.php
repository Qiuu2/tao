...
else
{
		...
		if(!empty($_GET['searchkey']))
		{
			if(trim($_GET['searchkey'])=="taskname")
			{
				if(!empty($_GET['searchsequence']))
				{
					...
					$sql = "SELECT DISTINCT * FROM task WHERE taskname LIKE '%".trim($_GET['searchvalue'])."%' AND tasktype = 12 OR tasktype=13 ";
			...
			if(trim($_GET['searchkey'])=="playtime")
			{
				...
				{					
					$sql = "SELECT DISTINCT * FROM task WHERE playtime >= '".trim($_GET['searchvalue'])."' AND tasktype = 12 OR tasktype=13 ";
	...
	{	
		...
		{
			$sql = "SELECT DISTINCT * FROM task WHERE tasktype = '12' OR tasktype='13' AND info = '' AND channel = 0 AND sec_task_id = 0 AND prepower=0 ";
	...
	while ($row = mysqli_fetch_array($result)) 
	{
		$array1[]=array(
						...
						"timelength"=>$row['timelength'],"exemodel"=>$row['exemodel'],"priority"=>($row['priority']%10),
					
						"timelengthtype"=>$row['timelengthtype']
					);
	}
	$smarty->assign("info",$array1);

	
	unset($array1,$row,$sql);
	$array2=array();
	...
	$smarty->display("systemtask/systemtaskManager.html");
