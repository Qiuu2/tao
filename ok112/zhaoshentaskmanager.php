...
else
{
		...
		if($_POST['searchkey']!="")
		{
			if($_POST['searchkey']=="taskname")
			{
				if($_POST['searchsequence']!="")
				{
					...
					{
						$sql="SELECT * FROM task WHERE tasktype IN (25)   and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY '$searchsequence' desc ";
					}
					else
					{
						$sql="SELECT * FROM task WHERE tasktype IN (25)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and  task_user_id='$userid' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY '$searchsequence' desc ";
					}
				}
				else
				{
					...
					if($_SESSION['username']=="admin")
					{
						$sql="SELECT * FROM task WHERE tasktype IN (25)    and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY playtime desc ";
					}
					else
					{
						$sql="SELECT * FROM task WHERE tasktype IN (25)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and   task_user_id='$userid' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY playtime desc ";
					}
				}
			}
			...
			if($_POST['searchkey']=="playtime")
			{
				if($_POST['searchsequence']!="")
				{
					if($_SESSION['username']=="admin")
					{	
						$sql="SELECT * FROM task WHERE tasktype IN (25)  and  playtime >= '".trim($_POST['searchvalue'])."' ORDER BY playtime desc ";
					}
					else
					{
					$sql="SELECT * FROM task WHERE tasktype IN (25)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid')  and task_user_id='$userid' and  playtime >= '".trim($_POST['searchvalue'])."' ORDER BY playtime desc ";
					}
				}
				else
				{
	...
	else
	{
		...
		{
		$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,username FROM task,book_admin WHERE task_user_id=book_admin.id AND tasktype IN (25)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid')  ORDER BY playtime desc";
		}
		else
		{
		$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,book_admin.username FROM task,book_admin WHERE task_user_id=book_admin.id  AND tasktype IN (25)  ORDER BY playtime desc";
		}
	}
...
/*
	...
	*/
	...
	$smarty->display("zhaoshengtask/FileTaskManager.html");
}
?>
