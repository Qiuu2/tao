...
else
{
	...
	if(!empty($_GET['searchvalue']))
	{
		...
		{
			$sql = "SELECT DISTINCT id,eventtype,eventname FROM camer_alarm WHERE eventname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ASC";
		}
		else
		{
			$sql = "SELECT DISTINCT id,eventtype,eventname FROM camer_alarm WHERE eventname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ASC";
		...
		{
			$sql = "SELECT DISTINCT id,eventtype,eventname FROM camer_alarm ORDER BY id ASC";
		}
	}
	else
	{
	...
	{
		$info[] = array("id"=>$row['id'],"eventtype"=>$row['eventtype'],"eventname"=>$row['eventname']);	
	}
	...
	$smarty->display("camer/alarm_event.html");
}
?>