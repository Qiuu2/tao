...
else
{
	...
	else
	{
		...
		if(empty($_GET['id']))
		{
			if($userid==1)
			{
				$sqls="SELECT id FROM filetaskfree WHERE userid='$userid' ";
	...
	if(!empty($_POST['searchvalue']))
	{
		if($_POST['searchkey']!="")
		{
			if($_POST['searchkey']=="taskname")
			{
				if($_POST['searchsequence']!="")
				{
					...
					$sql="SELECT * FROM task WHERE tasktype IN (2,7)   and parentid='$get_task_id' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY '$_GET[searchsequence]' desc ";
					}
					else
					{
					$sql="SELECT * FROM task WHERE tasktype IN (2,7)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and parentid='$get_task_id' and task_user_id='$userid' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY '$_GET[searchsequence]' desc ";
					}
				}
				else
				{
					...
					if($_SESSION['username']=="admin")
					{
					$sql="SELECT * FROM task WHERE tasktype IN (2,7)   and  parentid='$get_task_id' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY playtime desc ";
					}
					else
					{
					$sql="SELECT * FROM task WHERE tasktype IN (2,7)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and  parentid='$get_task_id' and task_user_id='$userid' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY playtime desc ";
					}
				}
			}
			...
			if($_POST['searchkey']=="playtime")
			{
				if($_POST['searchsequence']!="")
				{
					...
					$sql="SELECT * FROM task WHERE tasktype IN (2,7) and parentid='$get_task_id' and  playtime >= '".trim($_POST['searchvalue'])."' ORDER BY playtime desc ";
					}
					else
					{
					$sql="SELECT * FROM task WHERE tasktype IN (2,7)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and parentid='$get_task_id' and task_user_id='$userid' and  playtime >= '".trim($_POST['searchvalue'])."' ORDER BY playtime desc ";
					}
				}
				else
				{
		...
		{	
			...
			if($_SESSION['username']=="admin")
			{
				$sql="SELECT * FROM task WHERE tasktype IN (2,7)   and parentid='$get_task_id' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY projectstate,playtime asc ";
			}
			else
			{
				$sql="SELECT * FROM task WHERE tasktype IN (2,7)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') and parentid='$get_task_id' and task_user_id='$userid' and taskname LIKE '%".trim($_POST['searchvalue'])."%' ORDER BY  projectstate,playtime asc ";
			}
		}
	}
	else
	{
		if($_SESSION['username']!="admin")
		{
		$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,username FROM task,book_admin WHERE task_user_id=book_admin.id AND tasktype IN (2,7)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') AND (parentid=$get_task_id) ORDER BY  projectstate,playtime asc";
		}
		else
		{
		$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,book_admin.username FROM task,book_admin WHERE task_user_id=book_admin.id AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') AND (parentid=$get_task_id) AND tasktype IN (2,7)  ORDER BY  projectstate,playtime asc";
		}
	}
...
@mysqli_free_result($terminalresult);
	...
	$results = mysqli_query($con,"SELECT parentid,userid FROM filetaskfree WHERE id=$get_task_id");
		if ($rows = mysqli_fetch_array($results)) 
		{
			if(($rows['parentid']==0))
			{
		...
		{
			$getresults = mysqli_query($con,"SELECT id,name FROM filetaskfree ORDER BY id asc ");
		}
		else
		{
			$getresults = mysqli_query($con,"SELECT id,name FROM filetaskfree WHERE userid='$userid' ORDER BY id asc ");
		}
		$gettreeid=array();
...
/*
	...
	*/
	...
	$smarty->display("FileAd/FileTaskManager.html");
}
?>
