...
else
{
	...
	if(!empty($_GET['searchvalue']))
	{
		if(trim($_GET['searchkey']) =="name")
		{
			...
			{
				$sql = "SELECT startdate,enddate,playtime,taskid,taskname,tasktype,info,exemodel FROM task ";
				$sql.= "WHERE offlinetask LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY ".trim($_GET['searchsequence'])." DESC ";
			}
			else
			{
				$sql = "SELECT startdate,enddate,playtime,taskid,taskname,tasktype,info,exemodel FROM task WHERE offlinetask =1";
	...
	else
	{
		...
		{
		$sql = "SELECT startdate,enddate,playtime,taskid,taskname,tasktype,info,exemodel FROM task";
		$sql.= "WHERE offlinetask =1 AND task.task_user_id='$userid'";
		}
	}
	//ѯ
	...
	{
		$array[] = array("id"=>$row['taskid'],"name"=>$row['taskname'],"tasktype"=>$row['tasktype'],"info"=>$row['info']
		,"startdate"=>$row['startdate'],"enddate"=>$row['enddate'],"playtime"=>$row['playtime'],"exemodel"=>$row['exemodel']);
	}
	...
	$smarty->display("offlinetask/offline_task.html");
}
?>