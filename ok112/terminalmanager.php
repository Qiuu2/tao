...
else
{
	...
	if(trim($_GET['searchvalue'])!="")
	{
		if(trim($_GET['searchkey'])!="" && trim($_GET['searchsequence'])!="")
		{
			...
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and terminal.id in(select terminalid from terminalofgroup where groupid='$id') and terminal.terminalname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ";
			}
			
			if(trim($_GET['searchkey'])=="ip")
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and terminal.id in(select terminalid from terminalofgroup where groupid='$id') and terminal.ip LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ";
			}
		}
		else if(!empty($_GET['searchkey']) && empty($_GET['searchsequence']))
		{
			...
			if(trim($_GET['searchkey'])=="ip")
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and terminal.id in(select terminalid from terminalofgroup where groupid='$id') and terminal.ip LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id DESC ";
			}
		}
		else
		{
	...
	else
	{
		...
		{
			if($id== 0)
			$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal where terminal.typeid != '0'  ORDER BY netstate ASC ";
			else if($id==-1)
			{
				$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and terminal.id NOT IN(SELECT DISTINCT terminalofgroup.terminalid FROM terminalofgroup)  ORDER BY netstate ASC ";
		
			}
			else
			$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and terminal.id in(select terminalid from terminalofgroup where groupid='$id')  ORDER BY netstate ASC ";
		}
		else
		{
			if($id== 0)
				$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and terminal.id IN( SELECT userterminal.terminalid FROM userterminal WHERE userterminal.userid IN(SELECT id FROM book_admin WHERE book_admin.username = '$user_name')) ORDER BY netstate ASC ";
			else if($id==-1)
			{
				$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and  terminal.id IN( SELECT userterminal.terminalid FROM userterminal WHERE  userterminal.terminalid NOT IN(SELECT DISTINCT terminalofgroup.terminalid FROM terminalofgroup,serverplaystream WHERE serverplaystream.streamid=terminalofgroup.groupid AND userid='$user_id') AND userterminal.userid IN(SELECT id FROM book_admin WHERE book_admin.username = '$user_name')) ORDER BY netstate ASC ";
			
			}
			else
	  			$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.typeid != '0' and  terminal.id IN( SELECT userterminal.terminalid FROM userterminal,terminalofgroup WHERE terminalofgroup.groupid='$id' and terminal.id=terminalofgroup.terminalid and userterminal.userid IN(SELECT id FROM book_admin WHERE book_admin.username = '$user_name')) ORDER BY netstate ASC ";
		}
	}
...
//	@mysqli_free_result($result_terminal);
	...
	*/
	...
	$smarty->display("TerminalManager/terminalmanager.html");
}
?>