...
else
{
	...
	if(!empty($_GET['searchvalue']))
	{
		...
		{
			$sql = "SELECT DISTINCT keyid,keyname FROM shortcutkeytask WHERE keyname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ASC";
		}
		else
		{
			$sql = "SELECT DISTINCT keyid,keyname FROM shortcutkeytask WHERE keyname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ASC";
	...
	else
	{
		if($_SESSION['username']=="admin")
		{
			$sql = "SELECT DISTINCT keyid,keyname FROM shortcutkeytask";
		}
		else
		{
			$sql = "SELECT DISTINCT keyid,keyname FROM shortcutkeytask";
	...
	$medianame="";
		$sqls = "
SELECT media.name,media.typeid FROM shortcutkeytask,media WHERE shortcutkeytask.mediaid=media.id AND shortcutkeytask.keyid='$keyid'";
		$results=mysqli_query($con,$sqls) or die("Execute error".mysqli_error($con));
			while ($rows = mysqli_fetch_array($results)) 
			{
				...
				$medianame=$medianame.','.$rows['name'];
			
			}
	
		$info[] = array("id"=>$row['id'],"name"=>$medianame,"key"=>$row['keyid'],"typeid"=>$typeid,"keyname"=>$row['keyname']);
	...
	$smarty->display("keytask_mapping/keytask_mapping.html");
}
?>