...
$sql = "SELECT mediaoftask.id, MAX(sort) AS sort FROM mediaoftask WHERE mediaoftask.taskid = '$gettaskid'";
$result = mysqli_query($con,$sql) or die(mysqli_error($con));
if($row = mysqli_fetch_array($result))
{
	$getmaxid = $row['id'];
...
$currsql = "UPDATE mediaoftask SET sort = '$getmaxsort' WHERE mediaoftask.id = '$getid'";
mysqli_query($con,$currsql) or die(mysqli_error($con));
unset($currsql);

$maxsql = "UPDATE mediaoftask SET sort = '$getsort' WHERE mediaoftask.id = '$getmaxid'";
