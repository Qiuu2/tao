...
else
{
	...
	else
	{
		if($_SESSION['username']!="admin")
		{
			$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,username FROM task,book_admin WHERE task_user_id=book_admin.id AND tasktype IN (2,7)  AND task_user_id IN (SELECT id FROM book_admin WHERE id='$userid') AND (parentid=$get_task_id or parentid=0) ORDER BY playtime desc";
		}
		else
		{
			$sql="SELECT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,defaultvolume,task_user_id,book_admin.username FROM task,book_admin WHERE task_user_id=book_admin.id AND (parentid=$get_task_id or parentid=0) AND tasktype IN (2,7)  ORDER BY playtime desc";
		}
	}
