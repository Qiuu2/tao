...
else
{
	...
	if(trim($_GET['keyvalue'])!="")
	{
		...
		{
			$sql="SELECT * FROM soundgroupinfo WHERE soundgroupinfo.name LIKE '%".trim($_GET['keyvalue'])."%' ORDER BY  ".$_GET['orderby']." DESC ";
		}
		else if(!empty($_GET['searchkey']) && empty($_GET['orderby']))
		{
			$sql="SELECT * FROM soundgroupinfo WHERE soundgroupinfo.name LIKE '%".trim($_GET['keyvalue'])."%' ORDER BY  soundgroupinfo.id DESC ";
		}
		else if(empty($_GET['searchkey']) && !empty($_GET['orderby']))
		{
			$sql="SELECT * FROM soundgroupinfo WHERE soundgroupinfo.name LIKE '%".trim($_GET['keyvalue'])."%' ORDER BY  ".$_GET['orderby']." DESC ";
	...
	else
	{
		...
		$sql="SELECT * FROM soundgroupinfo WHERE id ORDER BY soundgroupinfo.id ASC ";
		else
		$sql="SELECT * FROM soundgroupinfo WHERE id ORDER BY soundgroupinfo.id ASC ";
	...
	{
		$info[] =array("streamid"=>$row['id'],"name"=>$row['name']);
	...
	*/
	...
	$smarty->display("zhaoshengManager/streammanager.html");
}
?>