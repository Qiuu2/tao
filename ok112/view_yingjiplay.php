...
else
{
	...
	$smarty->assign("Termianl_manager_type",$Termianl_manager_type);
	...
	get_terminal_type(13,$do_php_prompt['Terminal_not_support'],$terminal_id,1);
	$shotcut_info=array();
	//取源终端名称
	...
	if($row = mysqli_fetch_array($result))
	{
		if($row['typeid']==41)
		{
			$shotcut_sql = "SELECT terminalkeymaptask.taskid,task.taskname,task.defaultvolume,terminalkeymaptask.keyid,terminal.terminalname FROM task,terminalkeymaptask,terminal WHERE terminalkeymaptask.taskid = task.taskid AND terminal.typeid='41' AND terminal.id in($terminal_id) AND terminal.id=task.cmdargs ";
			...
			{
				$shotcut_info[] = array("id"=>$shotcut_row['taskid']."/".$shotcut_row['keyid'],"taskname"=>$shotcut_row['taskname'],"defaultvolume"=>$shotcut_row['defaultvolume'],"key"=>$shotcut_row['keyid'],"terminalname"=>$shotcut_row['terminalname'],"taskid"=>$shotcut_row['taskid']
				 );
			}
	...
	$smarty->display("TerminalManager/view_yingjiplay.html");
}
?>