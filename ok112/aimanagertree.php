...
function get_filetask_info($parentid,$user_id)
{
	...
	{
		$result_folders = mysqli_query($con,"SELECT deviceaddr,shibiedeviceid FROM ai_devicedemo ") or die(mysqli_error($con));
	 }
	 else
	 {
...
function get_ledtask_userinfo($con)
{
	...
	if($_SESSION['username']=="admin")
	{
		$sql_folder = "SELECT deviceaddr,shibiedeviceid FROM ai_devicedemo";
	 }
	 else
	 {
...
$str_js = "<html xmlns=\"http://www.w3.org/1999/xhtml\"><head><meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\"/>
		...
		while($row_folder = mysqli_fetch_array($result_folder))
		{
			...
			$user__name = $row_folder['username'];
					   ...
					   <span class=\"folder\">
						<a href=\"ai_people.php?shibiedeviceid=".$row_folder['shibiedeviceid']."&userid=".$row_folder['shibiedeviceid']."\" target=\"mediafile\" >".$row_folder['deviceaddr']."</a></span>";
						 $str_js.="</li></ul>";		
	    }
		 $str_js.="</ul>
	</div>
