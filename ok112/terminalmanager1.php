...
else
{
	...
	if(trim($_GET['searchvalue'])!="")
	{
		if(trim($_GET['searchkey'])!="" && trim($_GET['searchsequence'])!="")
		{
			...
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.terminalname LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ";
			}
			
			if(trim($_GET['searchkey'])=="ip")
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.ip LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id ";
			}
		}
		else if(!empty($_GET['searchkey']) && empty($_GET['searchsequence']))
		{
			...
			if(trim($_GET['searchkey'])=="ip")
			{
				$sql_terminal = "SELECT id,groupid,isselectcall,terminalname, typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.ip LIKE '%".trim($_GET['searchvalue'])."%' ORDER BY id DESC ";
			}
		}
		else
		{
	...
	if($user_name=="admin")
		$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal ORDER BY netstate ASC ";
	  else
	  	$sql_terminal = "SELECT id,groupid, terminalname,isselectcall,typeid, netstate, devicestate, taskstate, ip, volume, isspeech ,instancy,isrecord,skaddress,skdevtype,totalcapacity,resetcapacity,issponsor,shortcircuit,lopencircuit,ropencircuit,temperature,humidity FROM terminal WHERE terminal.id IN(SELECT terminalid FROM userterminal WHERE userterminal.userid IN(SELECT id FROM book_admin WHERE book_admin.username = '$user_name')) ORDER BY netstate ASC ";
	}
