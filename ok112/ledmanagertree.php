...
function get_filetask_info($parentid,$user_id)
{
	global $con;
	if($_SESSION['username']=="admin")
	{
		$result_folders = mysqli_query($con,"SELECT ledtaskfree.id,ledtaskfree.name FROM ledtaskfree where parentid =$parentid and userid='$user_id'") or die(mysqli_error($con));
	 }
	 else
	 {
	 $userid=$_SESSION['userid'];

	 $result_folders = mysqli_query($con,"SELECT ledtaskfree.id,ledtaskfree.name FROM ledtaskfree where parentid = $parentid and userid='$userid'") or die(mysqli_error($con));
	 } 
...
function get_ledtask_userinfo($con)
{
	
	$userid=$_SESSION['userid'];	
	if($_SESSION['username']=="admin")
	{
		$sql_folder = "SELECT book_admin.id,book_admin.username FROM book_admin WHERE id IN(SELECT userid FROM ledtaskfree) order by id";
	 }
	 else
	 {
...
$str_js = "<html xmlns=\"http://www.w3.org/1999/xhtml\"><head><meta http-equiv=\"content-type\" content=\"text/html; charset=utf-8\"/>
		...
		$result_folder = get_ledtask_userinfo($con);
		
		while($row_folder = mysqli_fetch_array($result_folder))
		{
			
			$user__name = $row_folder['username'];
		
			$str_js .=	"<li><span class=\"folder\"> <a href=\"ledtaskmanager.php?id=0&userid=".$row_folder['id']."\" target=\"mediafile\" >".$user__name."</a></span>";

				$result_0folder = get_filetask_info('0',$row_folder['id']);
			
				while($row_0folder = mysqli_fetch_array($result_0folder))
				{
					   $str_js.="<ul class=\"filetree\"><li class=\"closed\">
					   <span class=\"folder\">
						<a href=\"ledtaskmanager.php?id=".$row_0folder['id']."&userid=".$row_folder['id']."\" target=\"mediafile\" >".$row_0folder['name']."</a></span>";
					    $result_1folder = get_filetask_info($row_0folder['id'],$row_folder['id']);
						while($row_1folder = mysqli_fetch_array($result_1folder))
						{
						$str_js.="<ul class=\"filetree\"><li class=\"closed\">
					  <span class=\"folder\">
						<a href=\"ledtaskmanager.php?id=".$row_1folder['id']."&userid=".$row_folder['id']."\" target=\"mediafile\" >".$row_1folder['name']."</a></span>";
						 $str_js.="</li></ul>";
						}
						 $str_js.="</li></ul>";		
				}
