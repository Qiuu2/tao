...
function get_select_item_check(id,state)
{
    ...
    {
        var switch_con = 0;
        if(tree2.hasChildren(id)> 0)
        {
              var child_id =  tree2.getAllSubItems(id);          	
        ...
        else if(tree2.hasChildren(id)== 0)
        {
    ...
    {
        tree2.setItemText(get_all_item_id[x],get_all_item_txt[x],"");   
    }
 
    for(var x in sel_item_seq)
    {
       
            tree2.setItemText(sel_item_seq[x],"<font color='red'><b>"+(parseInt(x)+parseInt(1))+"</b>-|</font>"+tree2.getItemText(sel_item_seq[x]),"");       
