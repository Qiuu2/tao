...
else
{
	...
	//$terminalcount = get_terminal_num($type);
	//$smarty->assign("terminalcount",$terminalcount);
	...
	{	
		$get_username = trim($_GET['infoname']);
	...
	if($get_username != "")
	{
		$sqlss="SELECT MIN(taskid) FROM task WHERE info ='$get_username' AND tasktype='1'";
		$resultss=mysqli_query($con,$sqlss)or die(mysqli_error($con));
		if($rowss=mysqli_fetch_array($resultss))
		{
			$get_id=$rowss['0'];
	...
	$sql="SELECT prepower,startdate,enddate,exemodel,info,priority,task_user_id,defaultvolume,datasendmodel FROM task WHERE task.taskid ='$get_id' AND task.tasktype IN(1,15)";
	...
	if($row=mysqli_fetch_array($result))
	{
		$bellinfo = array(
							...
							"exemodel"=>$row['exemodel'],"info"=>$row['info'],"priority"=>$row['priority'],"defaultvolume"=>$row['defaultvolume'],"datasendmodel"=>$row['datasendmodel'],"task_user_id"=>$row['task_user_id']
						);
	}
	...
	$sql = "SELECT task.taskid,task.taskname,task.timelengthtype,task.timelength,task.playtime,mediaoftask.mediaid,task.priority,media.name,media.timelength ";
	
	$sql.= "FROM task,mediaoftask,media WHERE  (task.info = (SELECT task.info FROM task WHERE taskid = '$get_id' AND task.tasktype IN(1,15))) ";
	
	$sql.= "AND mediaoftask.taskid = task.taskid AND media.id = mediaoftask.mediaid AND task.tasktype IN(1,15) ORDER BY playtime asc";
	...
	{
		$lessoninfo[]=array(
								"taskid"=>$row['taskid'],"taskname"=>$row['taskname'],"timelengthtype"=>$row['timelengthtype'],"timelength"=>$row[3],"playtime"=>$row['playtime'],"mediaid"=>$row['mediaid'],"priority"=>$row['priority'],"name"=>$row['name'],"mediatimelength"=>$row[8]
							);
	}
...
/*	$results = mysqli_query($con,"SELECT DISTINCT ttssentence.name,ttssentence.sentenceid FROM ttssentence ");
	...
	$smarty->assign("taskid",$get_id);
	$smarty->assign("mediatimelength",$mediatimelength);
	$smarty->assign("admin_id",$_SESSION['admin_id']);

	$smarty->display("BellManager/modifybellall.html");
}
?>