...
else
{
	...
	unset($termianl_sql,$termianl_row);
	   ...
	   $smarty->assign("ledlist",$ledlist); 
		...
		//取媒体文�?
		$sql = "SELECT text FROM ledsentence WHERE mediaid IN(SELECT mediaid FROM mediaoftask,task WHERE task.taskid=mediaoftask.taskid and sec_task_id='$get_id' and tasktype=24) ORDER BY mediaseq ASC";
	...
	$led_task_sql = "SELECT terminalid,deviceid FROM ledoftask WHERE ledoftask.taskid in(select taskid from task where sec_task_id= '$get_id' and tasktype=24) ORDER BY terminalid ";
	...
	unset($led_task_sql,$led_termianl_row);
		...
		$smarty->display("FileAd/ModifyFileTask_form.html");
}
?>
