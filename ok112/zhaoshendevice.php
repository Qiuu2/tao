...
else
{
	...
	if(trim($_GET['keyvalue'])!="")
	{
		...
		{
			$sql="SELECT * FROM sounddevice WHERE sounddevice.name LIKE '%".trim($_GET['keyvalue'])."%' ORDER BY  ".$_GET['orderby']." DESC ";
		}
		else if(!empty($_GET['searchkey']) && empty($_GET['orderby']))
		{
			$sql="SELECT * FROM sounddevice WHERE sounddevice.name LIKE '%".trim($_GET['keyvalue'])."%' ORDER BY  sounddevice.id DESC ";
		}
		else if(empty($_GET['searchkey']) && !empty($_GET['orderby']))
		{
			$sql="SELECT * FROM sounddevice WHERE sounddevice.name LIKE '%".trim($_GET['keyvalue'])."%' ORDER BY  ".$_GET['orderby']." DESC ";
	...
	else
	{
		...
		$sql="SELECT * FROM sounddevice WHERE id ORDER BY sounddevice.id ASC ";
		else
		$sql="SELECT * FROM sounddevice WHERE id ORDER BY sounddevice.id ASC ";
	...
	{
		$info[] = array("id"=>$row['id'],"ip"=>$row['ip'],"devaddr"=>$row['devaddr'],"name"=>$row['name'],"groupid"=>$row['groupid'],"dbvalue"=>$row['dbvalue']);
		unset($array1);
	}
	...
	*/
	...
	$smarty->display("zhaoshengManager/sounddevice.html");
}
?>