...
function getmodepower($opt,$con)
{
	...
	&&$opt!="ttsfiletaskstart_msg"&&$opt!="ttsfiletaskstop_msg"&&$opt!="start_curr_tast_state"&&$opt!="stop_curr_tast_state")
	{
...
*/
	...
	if(!empty($username) && !empty($userpwd))
	{
		...
		if(mysqli_num_rows($result) == 1)
		{
			...
			/*
			$serverip = mysqli_query($con,"SELECT ip FROM serverbaseparam") or die(mysqli_error($con));
...
require_once("inc/config.inc.php");
	...
	mysqli_query($con,"LOCK TABLE media WRITE,filefolder WRITE,mediaoftask WRITE,shortcutkeytask WRITE,alarmgroupmap WRITE,camer_alarmofmedia WRITE");
		...
		$sqls = "SELECT id FROM media WHERE folderid IN($fordid)";
	$results = mysqli_query($con,$sqls) or die(mysqli_error($con));
		while($row = mysqli_fetch_array($results))
		{
...
function delfiletask_msg($con)
{
   ...
   //读取媒体任务
	...
	while($row=mysqli_fetch_array($result))
	{
		...
		if( ($row['id']==7||$row['id']==6||$row['id']==5 || $row['id'] == 1 || $row['id'] == 2 || $row['id'] == 3 || $row['id'] == 4) && ($_SESSION['admin_id']!="administrator") )
		{
			//=============================================================================================
...
function folderdel_msg($con)
{
	...
	{
		if($value == 5 || $value == 1 || $value == 2 || $value == 3 || $value == 4||$value == 6 ||$value == 7 )
		{
	...
	@mysqli_query($con,"START TRANSACTION");
	$sql_folder = "SELECT filename,id FROM media WHERE media.folderid IN ($get_folder_id)";
...
function useradd_msg($con)
{
	...
	mysqli_query($con,"LOCK TABLE book_admin WRITE,serverplaystream WRITE,terminalofgroup WRITE,userterminal WRITE");
...
//	@system($command);
	...
	$sqls="UPDATE serverbaseparam SET sounddetect = '$sounddetect',backup='$mastersalve'";
	mysqli_query($con,$sqls) or die(mysqli_error($con));
	if($mastersalve==0)
	{
		 $command = "sudo cp /opt/script/my.cnf.d/server.cnf /etc/my.cnf.d/server.cnf -rf";
		 @system($command);
	}
	else
	{
		$command = "sudo cp /opt/script/my.cnf.d/server-master.cnf /etc/my.cnf.d/server.cnf -rf";
		@system($command);
	//	$command = "sudo /usr/bin/mysqldump -u root --password='a9000db#!ht' -R -q --databases audioserver > /opt/script/all.sql";
		
	//	@system($command);

		$command = "sudo mv /opt/script/mysqldel2 /opt/script/mysqldel.sh -f";
		@system($command);
	...
	$command = "sudo rm -rf /etc/my.cnf.d/server-master.cnf";
	@system($command);
	
	$command = "sudo rm -rf /etc/my.cnf.d/server-slave.cnf";
	@system($command);
	$command = "sudo rm -rf /etc/my.cnf.d/server-clients.cnf";
	@system($command);
	
	$command = "sudo chmod 644 /etc/my.cnf.d/server.cnf";
	@system($command);
...
function terminaldel_msg($con)
{
	...
	mysqli_query($con,"UPDATE serverbaseparam SET adjusttime='0'");	
...
function del_yingjiplay($con)
{
	...
	mysqli_query($con,"START TRANSACTION");
	mysqli_query($con,"LOCK TABLE terminalkeymaptask WRITE,task WRITE,mediaoftask WRITE,ttssentence WRITE,media WRITE");
...
function set_ai_demo_msg($con)
{
	...
	if(mysqli_num_rows($result_taskmap) > 0)
	{
		...
		mysqli_query($con,"UPDATE ai_timetts SET time = '$time1',demo = '$ttsdemo1',enable = '$enabletts1',volume='$volume1' WHERE id = '1' ") or die(mysqli_error($con));
	
		mysqli_query($con,"UPDATE ai_timetts SET time = '$time2',demo = '$ttsdemo2',enable = '$enabletts2',volume='$volume2' WHERE id = '2' ") or die(mysqli_error($con));
		
		mysqli_query($con,"UPDATE ai_timetts SET time = '$time3',demo = '$ttsdemo3',enable = '$enabletts3',volume='$volume3' WHERE id = '3' ") or die(mysqli_error($con));
	
	}
	else
	{
		$sql = "INSERT INTO ai_timetts (id,time,demo,enable,volume)VALUES('1','$time1','$ttsdemo1','$enabletts1','$volume1')";
				
		mysqli_query($con,$sql) or die(mysqli_error($con));
		$sql = "INSERT INTO ai_timetts (id,time,demo,enable,volume)VALUES('2','$time2','$ttsdemo2','$enabletts2','$volume2')";
				
		mysqli_query($con,$sql) or die(mysqli_error($con));
		$sql = "INSERT INTO ai_timetts (id,time,demo,enable,volume)VALUES('3','$time3','$ttsdemo3','$enabletts3','$volume3')";
...
function modify_yingjiplay($con)
{
	...
	$gettaskid=0;
	mysqli_query($con,"LOCK TABLE terminalkeymaptask WRITE,termnalid WRITE,task WRITE");
...
/*	$taskuserid=$_GET['taskuserid'];
	...
	*/
	...
	$sqlfun.= "info = '$scheme' , defaultvolume = '$task_default_volume' , priority = '$priority_value[$i]',task_user_id='$original_task_userid[$i]' ,offlinestate='0' where task.sec_task_id = '$belltaskidarray[$i]' ";
		...
		mysqli_query($con,"LOCK TABLES terminaloftask WRITE,mediaofterminal WRITE,task WRITE");
...
/*	$taskuserid=$_GET['taskuserid'];
	...
	*/
	...
	$task_userid_sql = "SELECT priority FROM task WHERE task_user_id = $task_user_id AND taskid =$get_taskid";
	...
	for($i=0;$i<count($belltaskidarray);$i++)
	{
		...
		else
		{
			...
			{
			$sql = "INSERT INTO task (taskname,israndomplay,projectstate,timelengthtype,prepower,datasendmodel,state,startdate, enddate, ";
			
			$sql.= "playtime, exemodel,priority,tasktype,channel,bandrate,samplerate,cmd,cmdargs,playfileid,info,defaultvolume,task_user_id) ";
			
			$sql.= "VALUES('$coursenamearray[$i]', '0', '0', '$selectnum', '$prepower', '$datasendmodel', '0', '$startdate', '$enddate', ";
	...
	$sqlfun.= "info = '$scheme' , defaultvolume = '$task_default_volume' , priority = '$priority',task_user_id='$task_user_id' ,offlinestate='0' where task.sec_task_id = '$belltaskidarray[$i]' ";
		...
		else
		{
			...
			{
			$sqlfun = "INSERT INTO task (taskname, israndomplay, timelengthtype,prepower, datasendmodel, state, startdate, enddate, ";
			$sqlfun.= "playtime,exemodel,priority,tasktype,channel,bandrate,samplerate,cmd,cmdargs,playfileid,info,defaultvolume,task_user_id,sec_task_id) ";
			$sqlfun.= "VALUES('$coursenamearray[$i]', '0', '$selectnum', '$prepower', '$datasendmodel', '0', '$startdate', '$enddate', ";
			$sqlfun.= "'$getprefunctiontime','$exemodel','$priority','9','0','0','0','0','0','0','$scheme','$task_default_volume','$task_user_id','$getnewbellid' )";
			mysqli_query($con,$sqlfun) or die(mysqli_error($con));
			unset($sqlfun);
			}
	...
	$sqlfun.= "info = '$scheme' , defaultvolume = '$task_default_volume' , priority = '$priority',task_user_id='$task_user_id' ,offlinestate='0' where task.tasktype='1' and task.sec_task_id = '$hiddenbellnotaskarray[$k]' ";
				mysqli_query($con,$sqlfun) or die(mysqli_error($con));
				unset($sqlfun);
...
//	$priority = trim($row['level'])*10 + $priority;
	...
	**************************/
	...
	mysqli_query($con,"LOCK TABLES task WRITE,terminaloftask WRITE,mediaoftask WRITE,media WRITE,ledsentence WRITE,ledoftask WRITE");
...
//	$priority = trim($row['level'])*10 + $priority;
	...
	mysqli_query($con,"LOCK TABLE task WRITE,terminaloftask WRITE,mediaoftask WRITE,ledoftask WRITE,ledsentence WRITE,media WRITE");
	...
	if($tasktype == 3)
	{
		...
		$sql.="playfileid = '$playfileid' , defaultvolume = '$task_default_volume',sec_task_id='$get_taskid' ,offlinestate='0' WHERE task.sec_task_id = '$col_term_prepower_id' and tasktype = '8' ";
...
function zhaoshengdeviceadd_msg($con)
{
	...
	*/
	$sql = "INSERT INTO `sounddevice` (ip,devaddr,NAME,groupid,sendport) VALUES('$device_ip', '$device_addr','$devicename','0','$sendchanne') ";
...
function delsqldate($con)
{
	...
	mysqli_query($con,"DELETE FROM filefolder WHERE id>6");
	...
	mysqli_query($con,"DELETE FROM media");
	...
	mysqli_query($con,"DELETE FROM tempctrlnet2");
	...
	mysqli_query($con,"COMMIT");
	$command = "sudo rm -rf /backup/mediadata/*.mp3";
	@system($command);
}

function modify_cala_backup()
{
	$fp = fopen('/opt/script/mysqldel.sql', 'r+'); 
	if ($fp) { 
	$i = 0; 
	while (!feof($fp)) { 
	//修改第二行数据 
	$centent=fgetss($fp); 
	if(strpos($centent,'master_host') != false){
	//if ($i == 68) { 
		$lens=strlen($centent);
		$lenss=-$lens;
		$str1 = str_replace('@', '\'',$centent);
...
//32位
/*
function setmaster_backup($con,$model,$master_ip,$Slave_IP,$subnetmaskip,$ip,$offlineport,$servername,$slavename)
{
...
//64位
function setmaster_backup($con,$model,$master_ip,$Slave_IP,$subnetmaskip,$ip,$offlineport,$servername,$slavename,$backup)
{
	require_once("inc/socket_conf.php");
	$i=0;
	$create_socket_obj = new create_socket_class();
	...
	$command = "sudo sed -i '150c ".$servername."  ".$ip."/".$i."/eth0 ha-post' /usr/local/heartbeat/etc/ha.d/haresources ";	
	//@system($command);
	...
	$command = "sudo sed -i '4c ".$Slave_IP."  ".$slavename."' /etc/hosts";
	@system($command);
	$command = "sudo sed -i '211c node ".$servername."' /usr/local/heartbeat/etc/ha.d/ha.cf";
	@system($command);
	
	$command = "sudo sed -i '212c node ".$slavename."' /usr/local/heartbeat/etc/ha.d/ha.cf";
	@system($command);
	...
	if($model==1)
	{
		...
		$command = "sudo chmod 644 /etc/my.cnf.d/server.cnf";
	@system($command);
		$command = "sudo cp /opt/heartbeat/crontab-master /etc/crontab -rf";
		@system($command);	
		//$command = "sudo sed -i '1c /opt/script/sync.sh&' /opt/script/ini";
		//@system($command);
		$command = "sudo sed -i '1c ".$servername."' /etc/hostname";
		@system($command);
		
		$command = "sudo hostname ".$servername."";
		@system($command);
	//	$command = "sudo sed -i '3c change master to master_host='".$master_ip."',master_user='root',master_password='a9000db#\!ht';'  /opt/script/mysqldel.sql";
	//	$command = "sudo sed -i '69c change master to master_host=@".$master_ip."@,master_user=@root@,master_password=@a9000db#\!ht@;' /opt/script/mysqldel.sql";
		//@system($command);
		//modify_cala_backup();
	...
	{
		//$command = "mysql -u root --password='a9000db#!ht' -e 'source /opt/script/mysqldel2.sql'";
		//@system($command);
		$command = "sudo /usr/bin/mysqldump -u root --password='a9000db#!ht' -R -q --databases audioserver > /opt/script/all.sql";
		@system($command);
		$command = "sudo mv /opt/script/mysqldel /opt/script/mysqldel.sh -f";
		@system($command);
		$command = "sudo cp /opt/script/my.cnf.d/server-slave.cnf /etc/my.cnf.d/server.cnf -rf";
		@system($command);	
		$command = "sudo chmod 644 /etc/my.cnf.d/server.cnf";
	    @system($command);

		$command = "sudo cp /opt/heartbeat/crontab-slave /etc/crontab -rf";
		@system($command);	
		//$command = "sudo sed -i '69c change master to master_host=\'".$master_ip."\', master_user=\'root\', master_password=\'a9000db#\!ht\';'  /opt/script/mysqldel.sql";
		$command = "sudo sed -i '69c change master to master_host=@".$master_ip."@,master_user=@root@,master_password=@a9000db#\!ht@;' /opt/script/mysqldel.sql";
	//	$command = "sudo sed -i '3c change master to master_host='".$master_ip."',master_user='root',master_password='a9000db#\!ht';'  /opt/script/mysqldel.sql";
		...
		//$command = "sudo cp /opt/script/mysqldel.sql /var/www/html/ok112/mysqldel.sql -rf";
		///@system($command);	
		//$command = "sudo chmod 777 /var/www/html/ok112/mysqldel.sql";
		//@system($command);
		
		modify_cala_backup();
	
	//	$command = "sudo sed -i '/^master-host=/c master-host=".$master_ip."' /etc/my.cnf.d/server.cnf";
		//@system($command);
	...
	//	@system($command);
		...
		$command = "sudo sed -i '2c ntpdate -u ".$master_ip."' /opt/script/timeupdate.sh";
		@system($command);
			$command = "sudo sed -i '1c ".$slavename."' /etc/hostname";
		@system($command);
		$command = "sudo hostname ".$slavename."";
		@system($command);
	...
	//	@system($command);
		...
		//@system($command);
		$command = "sudo cp /opt/script/apprun-slave.sh /opt/script/apprun.sh  -rf";
		@system($command);
		//delsqldate($con);
	//	$command = "sudo service mysqld stop";
	//	@system($command);
...
function serveredit_msg($con)
{
	...
	$subnetmaskip;
	if(isset($_POST['subnetmaskip']))
	{	
		$subnetmaskip = trim($_POST['subnetmaskip']);
	}
	$ip;
	if(isset($_POST['ip']))
	{	
...
/*	$getsqlsss = "UPDATE serverbaseparam SET name=$servername,dataport=$_POST[dateport],ip=$_POST[ip],gateway=$_POST[gateway],port=$_POST[port],udpport=$_POST[udpport],maxbandwidth=$_POST[maxbandwidth],maxhttpconnections=$_POST[maxhttpconnections],subnetmask=$_POST[subnetmaskip],offlineport=$_POST[offlineport],backupmode=$servermodes,model=$servermodel,masterip=$master_ip,slaveip=$Slave_IP,slavename=$slavename";
*/
	
$getsql ="UPDATE `serverbaseparam` SET `name`='$servername',`dataport`='$_POST[dateport]',`ip`='$_POST[ip]',`gateway`='$_POST[gateway]',`port`='$_POST[port]',`udpport`='$_POST[udpport]',`maxbandwidth`='$_POST[maxbandwidth]',`maxhttpconnections`='$_POST[maxhttpconnections]',`subnetmask`='$_POST[subnetmaskip]',`offlineport`='$_POST[offlineport]',`backupmode`='$servermodes',`model`='$servermodel',`masterip`='$master_ip',`slaveip`='$Slave_IP',`slavename`='$slavename' ";

	mysqli_query($con,$getsql);	
	$listenport=trim($_POST['webport']);
	if($listenport!="")
	{
		$command = "sudo sed -i '52c LISTEN ".$listenport."' /usr/local/apache2/conf/httpd.conf";
		@system($command);
		$command = "sudo sed -i '193c ServerName localhost:".$listenport."' /usr/local/apache2/conf/httpd.conf";
		@system($command);
		$command = "sudo sed -i '45c <VirtualHost *:".$listenport.">' /usr/local/apache2/conf/extra/httpd-vhosts.conf";
		@system($command);
	}
	
	if($sdkport!="")
	{
		$commands = "sudo sed -i '40c url = \"http://".$ip.":".$listenport."/swagger-ui/dist/swagger1.json\";' /var/www/html/ok112/swagger-ui/dist/index.html";
		@system($commands);
		$command = "sudo sed -i '53c LISTEN ".$sdkport."' /usr/local/apache2/conf/httpd.conf";
		@system($command);
		$commands = "sudo sed -i '11c \"host\": \"".$ip.":".$sdkport."\",' /var/www/html/ok112/swagger-ui/dist/swagger1.json";
		@system($commands);
		$command = "sudo sed -i '24c <VirtualHost *:".$sdkport.">' /usr/local/apache2/conf/extra/httpd-vhosts.conf";
		@system($command);
	}
	...
	@system($command);
		...
		@system($commandgateway);
		$command = "sudo sed -i '2c route add default gw ".$gateway."' /opt/script/ha-post.sh";
		@system($command);
		$getip=$_POST['ip'];
	if($servermodel==1)
	{
	...
	else
	{
		...
		setmaster_backup($con,$servermodel,$master_ip,$Slave_IP,$subnetmaskip,$ip,$offlineport,$servername,$slavename,$backup);
...
function init_date_msg($con)
{
		...
		{
			$commands = "sudo /usr/bin/mysqldump -u root --password='a9000db#!ht' -R -q --databases audioserver > /opt/script/all.sql";
			@system($commands);
		...
		//	$command = "rm -fr /var/lib/mysql/mysql-bin*;rm -rf /var/lib/mysql/*.info; rm -rf /var/lib/mysql/relay*;rm -rf /var/lib/mysql/aria_*;rm -rf /var/lib/mysql/*-relay*;rm -rf /var/lib/mysql/ibtmp*;rm -rf /var/lib/mysql/ib_*;rm -rf /var/lib/mysql/*.pid;rm -rf /var/lib/mysql/audioserver";
			...
			@system($command);
			sleep(1);
			$command = "sudo rm -f /var/lib/mysql/*.info";
			@system($command);
			$command = "sudo rm -f /var/lib/mysql/aria_*";
			@system($command);
			//$command = "sudo rm -f /var/lib/mysql/*.pid";
			//@system($command);
			$command = "sudo rm -f /var/lib/mysql/mysql-bin*";
			@system($command);
			$command = "sudo rm -f /var/lib/mysql/relay*";
			@system($command);
			$command = "sudo rm -f /var/lib/mysql/ib_*";
			@system($command);			
			$command = "sudo rm -f /var/lib/mysql/*-relay*";
			@system($command);
			//$command = "sudo rm -f /var/lib/mysql/ibtmp*";
			//@system($command);
			*/
			$_SESSION['info'] = strtoupper($do_php_prompt['Successed']."---".$do_php_prompt['The_system_is_restarting']);//提示信息
			$_SESSION['url'] = "./login.php";
...
{	
	...
	//执行命令
	$command = "sudo registerserver ".$license_key."";
	//注册服务器
...
{	
	...
	//	$socket->send_data($_SESSION['serverip'],$msg);
		...
		$myfile = fopen("serial", "w");
		$txt = "[system]\n";
		fwrite($myfile, $txt);
		$txt = "startdate=".date("Y")."-".date("m")."-".date("d")."\n";
		fwrite($myfile, $txt);
		fclose($myfile);
		$command = "sudo chmod 777 /var/www/html/ok112/serial";
		@system($command);
		$command = "sudo cp /var/www/html/ok112/serial /etc -rf";
		@system($command);
		$command = "sudo chmod 777 /etc/serial";
		@system($command);
		$command="sudo reboot";
		@system($command);
		$_SESSION['info'] = strtoupper($do_php_prompt['Successed']);//提示信息
