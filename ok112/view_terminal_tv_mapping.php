...
else
{
	...
	$smarty->assign("Termianl_manager_Stop",$Termianl_manager_Stop);
	$smarty->assign("Termianl_tv_addr",$Termianl_tv_addr);
	...
	get_terminal_type(4,$do_php_prompt['Terminal_not_support'],$terminal_id,1);

	//取源终端名称
	$shotcut_sql = "SELECT cameramap.id,cameramap.ipaddr,terminal.terminalname FROM terminal,cameramap WHERE terminal.id=cameramap.terminalid AND terminal.id='$terminal_id'";
...
//$shotcut_result =   mysqli_query($con,$shotcut_sql."LIMIT $start,$NumOfPage")or die("Execute error".mysqli_error($con));
	...
	while($shotcut_row = mysqli_fetch_array($shotcut_result))
	{
		...
		$shotcut_info[] = array("id"=>$shotcut_row['id'],"ipaddr"=>$shotcut_row['ipaddr'],"terminalname"=>$shotcut_row['terminalname']);
	}
...
*/
	...
	$smarty->display("TerminalManager/view_terminal_tv_mapping.html");
}
?>