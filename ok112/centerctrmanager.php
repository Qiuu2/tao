...
else
{
		...
		if(!empty($_GET['searchkey']))
		{
			if(trim($_GET['searchkey'])=="taskname")
			{
				if(!empty($_GET['searchsequence']))
				{
					...
					{
						$sql = "SELECT DISTINCT * FROM task WHERE taskname LIKE '%".trim($_GET['searchvalue'])."%' AND tasktype = 23 ";	
						$sql.= "AND info = '' AND sec_task_id=0 AND channel = 0 AND bandrate=0 ORDER BY startdate, enddate DESC, playtime ASC ";
					}
					else
					{
						$sql = "SELECT DISTINCT * FROM task WHERE taskname LIKE '%".trim($_GET['searchvalue'])."%' AND tasktype = 23 ";	
						$sql.= "AND info = '' and task_user_id='$userid' AND sec_task_id=0 AND channel = 0 AND bandrate=0 ORDER BY startdate, enddate DESC, playtime ASC ";
					}
				}
				else
				{
			...
			if(trim($_GET['searchkey'])=="playtime")
			{
				...
				{	
					if($_SESSION['username']=="admin")
					{				
						$sql = "SELECT DISTINCT * FROM task WHERE playtime >= '".trim($_GET['searchvalue'])."' AND tasktype = 23 ";
						$sql.= "AND info = '' AND sec_task_id=0 AND channel = 0 AND bandrate=0 ORDER BY startdate, enddate DESC, playtime ASC ";
					}
					else
					{
	...
	{	
		if($_SESSION['username']=="admin")
		{
			$sql = "SELECT DISTINCT taskid,taskname,israndomplay,projectstate,state,startdate,enddate,playtime,timelength,exemodel,priority,tasktype,timelengthtype,playfileid,channel,defaultvolume,cmdargs,bandrate,samplerate,task_user_id,book_admin.username FROM task,book_admin WHERE task_user_id=book_admin.id AND tasktype = 23 AND channel = 0 AND sec_task_id = 0 AND prepower=0 ";
...
/*	//状态分页
	...
	*/
	...
	$smarty->display("centercontrol/centerctrManager.html");
