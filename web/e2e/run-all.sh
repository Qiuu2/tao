...
TESTS=("$@")
if [ ${#TESTS[@]} -eq 0 ]; then
  TESTS=(login-slider terminal-grid terminal-selection terminal-shortcut-targets terminal-shortcut-column media-upload alarm-mapping-channels remote-edit map audit-detail bell-one-tone bell-plan-ops dashboard-browse)
fi
