  ...
  <div class="table-box">
    <ProTable ref="proTableRef" :columns="columns" :request-api="getSoundGroupsApi" :data-callback="dataCallback" row-key="id">
      <template #tableHeader="scope">
        <div class="header-bar">
          ...
          <div class="header-left">
            <el-button type="primary" :disabled="!canEdit" @click="openCreate">{{ $t("noise.addZone") }}</el-button>
      ...
      <template #operation="s">
        ...
        <el-button type="primary" link @click="openTerminals(s.row)">{{ $t("noise.browseTerminals") }}</el-button>
...
const columns = reactive<ColumnProps<SoundGroup>[]>([
  ...
  { prop: "name", label: t("noise.zoneName"), minWidth: 400 },
  { prop: "operation", label: t("common.operation"), fixed: "right", width: 230 }
]);
...
const openEditById = async (raw: (string | number)[]) => {
  const ids = toIds(raw);
  if (ids.length !== 1) return ElMessage.warning(t("noise.pickOneZone"));
...
const openTerminals = async (row: SoundGroup) => {
  const { data } = await getSoundGroupApi(row.id);
  termDlg.title = t("noise.terminalsOf", { name: data.name });
...
const doDelete = async (raw: (string | number)[]) => {
  ...
  ElNotification({
    ...
    message: t("noise.zoneDeletedSummary", {
      zones: data.deleted.length,
      terminals: data.resetTerminals,
      devices: data.resetDevices
    }),
    type: "success"
  });
  refresh();
