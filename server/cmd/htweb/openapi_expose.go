...
var keyDenied = map[string]string{
	...
	"GET /api/changes": "长轮询，专给浏览器「不刷新也跟着变」用的。" +
		"它一挂就是 25 秒，而且只回几个哈希、不回任何数据 —— " +
		"程序要知道终端和任务的状态，直接调 /openapi/v1 那边的查询接口，" +
		"那才是给程序设计的形状",
...
var exposedGroups = []exposedGroup{
	...
	},
	{
		...
		APIs: []exposedAPI{
			...
			{"POST /api/typed-tasks/{kind}", "新建分类任务", "按类别（powerplay/admpriv/ttspriv/telephonepriv/soundtaskpriv）", true},
	...
	},
	{
		...
		APIs: []exposedAPI{
			...
			{"POST /api/enable-plans", "新建启用计划", "enablepriv", true},
			{"PUT /api/enable-plans/{id}", "修改启用计划", "enablepriv", true},
			{"DELETE /api/enable-plans", "删除启用计划", "enablepriv", true},
		},
	},
	{
		Name: "报警",
	...
	},
	{
		...
		APIs: []exposedAPI{
			...
			{"POST /api/sound/devices", "新建噪声设备", "noisedevpriv", true},
			{"PUT /api/sound/devices/{id}", "修改噪声设备", "noisedevpriv", true},
			{"DELETE /api/sound/devices", "删除噪声设备", "noisedevpriv", true},
			...
			{"POST /api/sound/groups", "新建声场分区", "soundzonepriv", true},
			{"PUT /api/sound/groups/{id}", "修改声场分区", "soundzonepriv", true},
			{"DELETE /api/sound/groups", "删除声场分区", "soundzonepriv", true},
		},
	},
	{
		Name: "云广播与离线",
		...
		APIs: []exposedAPI{
			...
			{"POST /api/offline/media", "下发媒体到终端", "offlinepriv", true},
			{"POST /api/offline/tasks", "下发任务到终端", "offlinepriv", true},
			{"PUT /api/offline/stop", "停止下发", "offlinepriv", true},
			{"GET /api/cloud/terminals", "云广播终端列表", "登录即可", false},
			{"GET /api/cloud/terminals/{id}/inventory", "终端上有哪些媒体/任务", "登录即可", false},
			{"POST /api/cloud/bulk", "云广播终端批量动作（含全部清除）", "cloudterminalpriv", true},
			...
			{"POST /api/transfer/bulk", "云广播任务批量动作（含删除离线音乐）", "transferpriv", true},
			...
			{"POST /api/transfer/server-bulk", "把服务器任务下发到它自己的终端", "transferpriv", true},
		},
	},
	{
		Name: "遥控任务",
