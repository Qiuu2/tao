...
//
// 这一页在旧版上做四件事，新版四件都做，但第 ① 件换了一套实现：
//
//	① 显示 / 修改服务器系统时间        —— 见 clock.go，能不能改是**每次现探**的
...
// 这种事不该藏在一个 Web 表单后面。
//
// 所以新版的做法是：默认只**显示**服务器当前时间、时区与运行时长，
// 让运维能一眼看出有没有跑偏；「设置服务器时间」这两个按钮要不要点得动，
// 由 clock.go 的 CanSet() 每次请求现探 —— 探的是「有没有那条受限的免密 sudo 规则」
// 和「NTP 是不是在跑」。规则由 deploy/install-sudoers.sh 装，只放开
// `timedatectl set-time` 与 `set-ntp` 两条。没装就置灰，并如实回报缺什么，
// 不猜密码也不假装成功。
//
