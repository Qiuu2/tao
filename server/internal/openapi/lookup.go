...
// # 两种都收，但对外一律推荐 id
//
// 收名字是因为对方系统（教务、门禁、值班表）里存的就是**名字**：
// 逼他们先调一次列表接口换成 id，等于把我们的实现细节变成他们的工作量。
// 所以名字照收，能对上就用。
//
// 但文档和平台上一律把 **id 写在前面**，因为只有 id 是唯一的：
//
//	media.name              没有唯一索引，应用层也没拦重名
//	terminal.terminalname   同上
//	task.taskname           同上
//	zone / 任务分组 / 媒体目录  新建时挡了重名，用名字是安全的
//	作息方案                 根本没有 id，只能用名字
//
// 也就是说两台终端都叫「操场音箱」在这套系统里完全合法。那一天到来时，
// 对方那段写死名字的代码会收到「找到多个同名终端」—— 而这通常发生在
// 上线半年之后。推荐 id 不是嫌名字麻烦，是不想把这颗雷留给对接方。
//
