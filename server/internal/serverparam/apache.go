...
//
// **端口**这两项新版只读不写：改 Apache 端口要按行号 sed 配置文件（D-209），
// 一旦行数变化就会写坏 httpd.conf 让整个 Web 起不来，这个险不值得冒。
// 页面上它们置灰展示，真要改由运维直接改配置。
//
// ⚠ 例外是 swagger1.json 里 `"host": "<ip>:<端口>"` 的 **IP 那一半**：
//   它记的就是服务器地址，改 IP 时要跟着改，否则对外 API 文档还指着旧地址。
//   那一步在 syncfiles.go 里做，认 "host" 这个键、不按行号，且**端口原样留着**
//   （旧版是整行写死成 `"host": "<ip>:99"`，把改过的 sdk 端口又拽回 99）。
